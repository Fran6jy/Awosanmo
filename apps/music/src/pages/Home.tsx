...
const playMix = (m: Mix) => api<{ tracks: Track[] }>(`/api/music/mixes/${encodeURIComponent(m.id)}`).then((r) => playQueue(r.tracks, 0, { kind: "mix", name: m.name, id: m.id }));
const playMood = (m: Mood) => api<{ tracks: Track[] }>(`/api/music/moods/${m.id}`).then((r) => playQueue(r.tracks, 0, { kind: "mood", name: m.name, id: m.id }));
